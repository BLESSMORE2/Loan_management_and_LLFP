...
def process_entry(entry, cash_flow_cache, uses_discounting):
    cash_flow_records = cash_flow_cache.get(entry.n_account_number, [])
    if uses_discounting:
        total_cash_shortfall_pv = sum(record['n_cash_shortfall_pv'] for record in cash_flow_records if record['n_cash_shortfall_pv']) or 0
        total_12m_cash_shortfall_pv = sum(record['n_12m_cash_shortfall_pv'] for record in cash_flow_records if record['n_12m_cash_shortfall_pv']) or 0
        entry.n_lifetime_ecl_ncy = total_cash_shortfall_pv
        entry.n_12m_ecl_ncy = total_12m_cash_shortfall_pv
    else:
        total_cash_shortfall = sum(record['n_cash_shortfall'] for record in cash_flow_records if record['n_cash_shortfall']) or 0
        total_12m_cash_shortfall = sum(record['n_12m_cash_shortfall'] for record in cash_flow_records if record['n_12m_cash_shortfall']) or 0
...
def update_ecl_based_on_cash_shortfall(n_run_key, fic_mis_date, uses_discounting):
    try:
        ...
        cash_flow_cache = {
            entry['v_account_number']: list(fsi_Financial_Cash_Flow_Cal.objects.filter(
                n_run_skey=n_run_key,
                fic_mis_date=fic_mis_date,
                v_account_number=entry['v_account_number']
            ).values('n_cash_shortfall_pv', 'n_12m_cash_shortfall_pv', 'n_cash_shortfall', 'n_12m_cash_shortfall'))
            for entry in reporting_lines.values('v_account_number')
        }
        ...
        with transaction.atomic():
            for i in range(0, len(updated_entries), BATCH_SIZE):
                FCT_Reporting_Lines.objects.bulk_update(
                    updated_entries[i:i + BATCH_SIZE], ['n_lifetime_ecl_ncy', 'n_12m_ecl_ncy']
                )

        save_log('update_ecl_based_on_cash_shortfall', 'INFO', f"Successfully updated {len(updated_entries)} records for ECL based on cash shortfall for run key {n_run_key} and MIS date {fic_mis_date}.")
    except Exception as e:
...
def update_ecl_based_on_internal_calculations(n_run_key, fic_mis_date):
    try:
        ...
        save_log('update_ecl_based_on_internal_calculations', 'INFO', f"Successfully updated {len(updated_entries)} records for ECL based on internal calculations for run key {n_run_key} and MIS date {fic_mis_date}.")
    except Exception as e:
