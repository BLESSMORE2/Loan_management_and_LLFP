...
def update_cash_flow_with_pd_buckets_v6(fic_mis_date, n_run_skey, max_workers=5):
    """
        ...
        try:
            ...
            if account_data:
                ...
                if pd_records.exists():
                    # Step 2: Align the buckets using v_amrt_term_unit and v_cash_flow_bucket_unit
                    pd_first_record = pd_records.first()
                    adjusted_bucket_id = adjust_bucket_id(
                        account_data.v_amrt_term_unit,
                        pd_first_record.v_cash_flow_bucket_unit,
                        cash_flow.n_cash_flow_bucket_id
                    )

                    # Step 3: Use V_PD_TERM_STRUCTURE_TYPE to determine if rating-based (R) or delinquency band-based (D)
                    if pd_first_record.v_pd_term_structure_type == 'R':
                        # PD based on Rating (for Rating-based PDs)
                        pd_record = pd_records.filter(
                            v_int_rating_code=account_data.n_credit_rating_code,
                            v_cash_flow_bucket_id=adjusted_bucket_id
                        ).first()
                    elif pd_first_record.v_pd_term_structure_type == 'D':
                        # PD based on Delinquency Band (for Delinquency-based PDs)
                        delinquent_band_code = get_delinquent_band_code(account_data.n_delinquent_days)
                        pd_record = pd_records.filter(
                            v_delq_band_code=delinquent_band_code,
                            v_cash_flow_bucket_id=adjusted_bucket_id
                        ).first()

                    # Step 4: Assign PD values to cash flow records for the given bucket
                    if pd_record:
                        # Assign cumulative loss rate and impaired prob
...
def adjust_bucket_id(v_amrt_term_unit, v_cash_flow_bucket_unit, n_cash_flow_bucket_id):
    """
    This function adjusts the cash flow bucket ID based on the term unit of the account (v_amrt_term_unit)
    and the cash flow bucket unit in the PD table (v_cash_flow_bucket_unit).
    For example, if v_amrt_term_unit is quarterly (Q) and v_cash_flow_bucket_unit is monthly (M),
    then every 3 buckets in a monthly system should correspond to 1 bucket in a quarterly system.
    """
    term_unit_to_months = {
        'M': 1,  # Monthly
        'Q': 3,  # Quarterly
    ...
    # Convert term unit and cash flow bucket unit into equivalent months
    amrt_term_in_months = term_unit_to_months.get(v_amrt_term_unit, 1)  # Default to monthly if not found
    bucket_unit_in_months = term_unit_to_months.get(v_cash_flow_bucket_unit, 1)  # Default to monthly if not found

    # Calculate the adjusted bucket ID
    adjusted_bucket_id = n_cash_flow_bucket_id // (amrt_term_in_months // bucket_unit_in_months)

    return max(1, adjusted_bucket_id)  # Ensure the bucket ID doesn't go below 1
...
def get_delinquent_band_code(delinquent_days):
    """
    This function returns the delinquent band code based on the number of delinquent days.
    """
    if delinquent_days == 0:
        return 'Active'
    elif 1 <= delinquent_days <= 30:
        return '1-30'
    elif 31 <= delinquent_days <= 60:
        return '31-60'
    elif 61 <= delinquent_days <= 89:
        return '61-89'
    elif delinquent_days >= 90:
        return '91+'
    return None  # No delinquent band found


# Example usage with multi-threading:
update_cash_flow_with_pd_buckets_v6(fic_mis_date='2024-09-17', n_run_skey=12345, max_workers=10)
