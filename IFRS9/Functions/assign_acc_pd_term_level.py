...
def calculate_pd_for_accounts(fic_mis_date):
    ...
    Compatible with MariaDB, MySQL, PostgreSQL, and Oracle.
    """
    try:
        with transaction.atomic(), connection.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                SET 
                    sd.n_twelve_months_pd = (
                        SELECT pd12.n_cumulative_default_prob
                        FROM ldn_pd_term_structure ts
                        JOIN fsi_pd_interpolated pd12 
                          ON pd12.v_pd_term_structure_id = ts.v_pd_term_structure_id
                         AND pd12.v_int_rating_code = sd.n_credit_rating_code
                         AND pd12.v_cash_flow_bucket_id = 
                             CASE 
                               WHEN TIMESTAMPDIFF(MONTH, sd.fic_mis_date, sd.d_maturity_date) <= 12 THEN FLOOR(
                                   GREATEST(
                               ...
                               ELSE FLOOR(
                                   GREATEST(
                                       CASE 
                                         WHEN ts.v_pd_term_frequency_unit = 'M' THEN 12
                                         WHEN ts.v_pd_term_frequency_unit = 'Q' THEN 4
                                         WHEN ts.v_pd_term_frequency_unit = 'H' THEN 2
                                         WHEN ts.v_pd_term_frequency_unit = 'Y' THEN 1
                                         ELSE 12
                                       END,
                                       1
                                   )
                        ...
                        WHERE ts.v_pd_term_structure_type = 'R'
                          AND ts.v_pd_term_structure_id = sd.n_pd_term_structure_skey
                          AND pd12.v_cash_flow_bucket_id IS NOT NULL
                        FETCH FIRST 1 ROWS ONLY
                    ),
                    sd.n_lifetime_pd = (
                        SELECT pd_lt.n_cumulative_default_prob
                        FROM ldn_pd_term_structure ts
                        JOIN fsi_pd_interpolated pd_lt 
                          ON pd_lt.v_pd_term_structure_id = ts.v_pd_term_structure_id
                         AND pd_lt.v_int_rating_code = sd.n_credit_rating_code
                         AND pd_lt.v_cash_flow_bucket_id = FLOOR(
                             GREATEST(
                                 TIMESTAMPDIFF(MONTH, sd.fic_mis_date, sd.d_maturity_date),
                                 1
                             )
                        ...
                        WHERE ts.v_pd_term_structure_type = 'R'
                          AND ts.v_pd_term_structure_id = sd.n_pd_term_structure_skey
                          AND pd_lt.v_cash_flow_bucket_id IS NOT NULL
                        FETCH FIRST 1 ROWS ONLY
                    )
                WHERE sd.fic_mis_date = %s AND sd.d_maturity_date IS NOT NULL;
            """, [fic_mis_date])
            ...
            cursor.execute("""
                ...
                SET 
                    ...
                    sd.n_lifetime_pd = (
                        ...
                        JOIN fsi_pd_interpolated pd_lt 
                          ON pd_lt.v_pd_term_structure_id = ts.v_pd_term_structure_id
                         AND pd_lt.v_delq_band_code = sd.n_delq_band_code
                         AND pd_lt.v_cash_flow_bucket_id = FLOOR(
                             GREATEST(
                                 TIMESTAMPDIFF(MONTH, sd.fic_mis_date, sd.d_maturity_date),
                                 1
