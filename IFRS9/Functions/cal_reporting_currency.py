...
def update_reporting_lines(fic_mis_date, exchange_rate_dict, target_currency_code):
    ...
    def process_entry(line):
        if line.v_ccy_code == target_currency_code:
            ...
            exchange_rate = exchange_rate_dict.get(exchange_rate_key)

            if exchange_rate is None:
                error_msg = f"Exchange rate not found for {line.v_ccy_code} to {target_currency_code}"
                unique_error_logs[error_msg] = unique_error_logs.get(error_msg, 0) + 1
                return line
    ...
    with transaction.atomic():
        for i in range(0, len(updated_entries), batch_size):
            FCT_Reporting_Lines.objects.bulk_update(updated_entries[i:i + batch_size], [
    ...
    for error_message, count in unique_error_logs.items():
        save_log('update_reporting_lines', 'ERROR', f"{error_message} occurred {count} times.")
